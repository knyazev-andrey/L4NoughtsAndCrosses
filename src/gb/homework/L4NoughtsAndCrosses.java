package gb.homework;
/**
 * @author Andrey Knyazev
 * @version dated by 28.05.2018
 */

import java.util.Random;
import java.util.Scanner;

public class L4NoughtsAndCrosses {
    public static Scanner scanner = new Scanner(System.in);
    public static Random random = new Random();

    public static char[][] map;
    public static final int SIZE = 4;
    public static final char DOT_EMPTY = '*';
    public static final char DOT_PL = 'X';
    public static final char DOT_AI = 'O';

    public static void initMap() {
        map = new char[SIZE][SIZE];
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                map[i][j] = DOT_EMPTY;
            }
        }
    }

    public static void printMap() {
        for (int i = 0; i < SIZE + 1; i++) {
            System.out.print(i + " ");
        }
        System.out.println();
        for (int i = 0; i < SIZE; i++) {
            System.out.print((i + 1) + " ");
            for (int j = 0; j < SIZE; j++) {
                System.out.print(map[i][j] + " ");
            }
            System.out.println();
        }
        System.out.println();
    }

    public static boolean isMapFull() {
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (map[i][j] == DOT_EMPTY) return false;
            }
        }
        return true;
    }

    public static void humanTurn() {
        int x, y;
        do {
            System.out.println("Введите координаты в формате X Y");
            x = scanner.nextInt() - 1;
            y = scanner.nextInt() - 1;
        } while (!isCellValid(x, y));
        map[y][x] = DOT_PL;
    }

    public static void aiTurn() {
        int x, y;
        do {
            x = random.nextInt(SIZE);
            y = random.nextInt(SIZE);
        } while (!isCellValid(x, y));
        System.out.println("Компьютер походил в точку " + (x + 1) + " " + (y + 1));
        map[y][x] = DOT_AI;
    }

    public static boolean isCellValid(int x, int y) {
        if (x < 0 || x >= SIZE || y < 0 || y >= SIZE) return false;
        if (map[y][x] == DOT_EMPTY) return true;
        return false;
    }

    public static boolean checkWin(char dot) {
        for (int i = 0; i < map.length; i++) {
            if (map[i][0] == dot && map[i][1] == dot && map[i][2] == dot && map[i][3] == dot) return true;
            if (map[0][i] == dot && map[1][i] == dot && map[2][i] == dot && map[3][i] == dot) return true;
        }
        int counterOne = 0;
        int counterTwo = 0;
        for (int i = 0; i < map.length - 1; i++) {
            for (int j = 0; j < map.length - 1; j++) {
                if (i == j) {
                    if (map[i][j] == dot && map[i + 1][j + 1] == dot) counterOne++;
                    if (map[map.length - 1 - i][j] == dot && map[map.length - 2 - i][j + 1] == dot) counterTwo++;
                }
                if (counterOne == 3 || counterTwo == 3) return true;
            }
        }
        return false;
    }


    public static void main(String[] args) {
        initMap();
        printMap();
        while (true) {
            humanTurn();
            printMap();
            if (checkWin(DOT_PL)) {
                System.out.println("Победил человек");
                break;
            }
            if (isMapFull()) {
                System.out.println("Ничья");
                break;
            }
            aiTurn();
            printMap();
            if (checkWin(DOT_AI)) {
                System.out.println("Победил Искуственный Интеллект");
                break;
            }
            if (isMapFull()) {
                System.out.println("Ничья");
                break;
            }
        }
        System.out.println("Игра закончена");
    }
}